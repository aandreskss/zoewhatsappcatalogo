...
export default async function VisitantesPage() {
  ...
  const { data: events } = await supabase
    ...
    .select("session_id, event_type, metadata, utm_source, utm_campaign, created_at")
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          { label: "Compradores", value: buyers, sub: total > 0 ? `${Math.round((buyers / total) * 100)}% conversión` : "—" },
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      <span className="text-sm font-medium text-[#29252A]">
                        {session.entryPage ? `Entró por /${session.entryPage}` : "Entrada desconocida"}
                      </span>
                    ...
                    <p className="text-xs text-[#29252A]/50">
                      ...
                      {session.utmSource ? ` · ${session.utmSource}` : " · directo"}
