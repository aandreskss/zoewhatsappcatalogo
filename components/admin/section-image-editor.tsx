...
export function SectionImageEditor({ id, imageUrl: initialImageUrl }: Props) {
  ...
  return (
    <div className="mt-1 flex flex-col gap-3 rounded-xl border border-[#EBE0E7] bg-white p-4">
      {/* Header */}
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          {busy && (
            <span className="text-xs text-[#29252A]/50">
      ...
      {preview ? (
        ...
        <div
          className="relative flex flex-col items-center gap-3 rounded-xl border-2 border-dashed border-[#EBE0E7] px-6 py-10 text-center"
          style={{ minHeight: 140 }}
        >
          {uploading ? (
            <>
              <div className="h-8 w-8 animate-spin rounded-full border-2 border-[#7B1847] border-t-transparent" />
              <span className="text-sm text-[#29252A]/60">Subiendo imagen…</span>
            </>
          ) : (
            <>
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-[#29252A]/50">
                  JPG, PNG o WEBP · máx. 10 MB · 16:9 recomendado
                </p>
              </div>
            </>
          )}

          {/* Input absolutamente posicionado, cubre toda la zona de drop */}
          <input
            type="file"
